...
class ReplyThread extends StatelessWidget {
  ...
  const ReplyThread({super.key, required this.selectedMessage, required this.chatStore});

  @override
  Widget build(BuildContext context) {
    final replyParent = chatStore.messages.firstWhereOrNull(
      (message) => message.tags['id'] == selectedMessage.tags['reply-parent-msg-id'],
    );
    ...
    final replyName = replyUserLogin != null ? getReadableName(replyDisplayName!, replyUserLogin) : replyDisplayName;
    ...
    return SizedBox(
      ...
      child: Observer(
        builder: (context) {
          return MediaQuery(
            data: MediaQuery.of(context).copyWith(textScaler: TextScaler.linear(chatStore.settings.messageScale)),
            child: DefaultTextStyle(
              ...
              child: Column(
                ...
                children: [
                  const SectionHeader('Replies', isFirst: true, padding: EdgeInsets.fromLTRB(12, 0, 12, 8)),
                  if (replyParent != null)
                    ChatMessage(ircMessage: replyParent, chatStore: chatStore)
                  else
                    Padding(
                      padding: const EdgeInsets.fromLTRB(12, 0, 12, 8),
                      child: Text('$replyName: $replyBody', style: const TextStyle(fontWeight: FontWeight.w500)),
                    ),
                  Flexible(
                    child: FrostyScrollbar(
                      child: ListView(
                        ...
                        children: chatStore.messages
                            .where(
                              (message) =>
                                  message.tags['reply-parent-msg-id'] == selectedMessage.tags['reply-parent-msg-id'],
                            )
                            .map(
                              (message) => ChatMessage(
                                isModal: true,
