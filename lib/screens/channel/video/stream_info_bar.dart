...
class StreamInfoBar extends StatelessWidget {
  ...
  /// When set, called on avatar tap instead of the default VOD list [Navigator.push].
  ...
  TextStyle _getBaseTextStyle(BuildContext context, double fontSize, FontWeight fontWeight) {
  ...
  TextStyle _getSecondaryTextStyle(BuildContext context, double fontSize) {
    return context.textTheme.bodyMedium?.copyWith(
          ...
          color: textColor?.withValues(alpha: 0.7) ?? context.bodySmallColor?.withValues(alpha: 0.7),
          shadows: showTextShadows ? _textShadow : null,
        ) ??
        const TextStyle();
  }
  ...
  Widget build(BuildContext context) {
    final streamTitle = isOffline ? (offlineChannelInfo?.title.trim() ?? '') : (streamInfo?.title.trim() ?? '');
    ...
    final userId = isOffline ? (offlineChannelInfo?.broadcasterId ?? '') : (streamInfo?.userId ?? '');
    ...
    final profilePicture = Container(
      decoration: isInSharedChatMode
          ...
          : null,
      child: Padding(
        padding: isInSharedChatMode ? const EdgeInsets.all(1.5) : EdgeInsets.zero,
        child: ProfilePicture(userLogin: userLogin, radius: 16),
      ),
    );
                  ...
                  Row(
                    children: [
                      if (isOffline && showOfflineIndicator) ...[
                        Text('Offline', style: _getSecondaryTextStyle(context, secondLineSize)),
                      ],
                      if (isOffline && showCategory && (offlineChannelInfo?.gameName.isNotEmpty ?? false)) ...[
                        if (showOfflineIndicator) const SizedBox(width: 8),
                        Icon(
                          Icons.gamepad,
                          size: secondLineSize,
                          color: (textColor ?? context.bodySmallColor)?.withValues(alpha: 0.7),
                          shadows: _iconShadow,
                        ),
                        ...
                        Flexible(
                          child: Tooltip(
                            ...
                            child: tappableCategory
                                ? GestureDetector(
                                    onDoubleTap: () {
                                      if (offlineChannelInfo?.gameId.isNotEmpty ?? false) {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                CategoryStreams(categoryId: offlineChannelInfo?.gameId ?? ''),
                                          ),
                                        );
                                      }
                                    },
                                    child: Text(
                                      ...
                                      style: _getSecondaryTextStyle(context, secondLineSize),
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                  )
                                : Text(
                        ...
                        if (!isOffline && showUptime) ...[
                          Uptime(
                            startTime: streamInfo?.startedAt ?? DateTime.now().toIso8601String(),
                            style: _getBaseTextStyle(context, secondLineSize, FontWeight.w500),
                          ),
                        ...
                        if (!isOffline && showCategory && (streamInfo?.gameName.isNotEmpty ?? false)) ...[
                          Icon(
                            Icons.gamepad,
                            size: secondLineSize,
                            color: textColor ?? context.bodySmallColor,
                              ...
                              child: tappableCategory
                                  ? GestureDetector(
                                      onDoubleTap: () => Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) => CategoryStreams(categoryId: streamInfo?.gameId ?? ''),
                                        ),
                                      ),
                                      child: Text(
                                        streamInfo?.gameName ?? '',
